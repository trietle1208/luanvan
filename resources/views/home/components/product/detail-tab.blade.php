                ...
                @foreach($product->para as $para)
                    @foreach($product->detail as $detail)
                        @if($para->ts_id == $detail->ts_id)
                            <span class=""><b>{{ $para->ts_tenthongso }} :</b> {{ $detail->chitietthongso }}</span><hr>
                        @endif
                    @endforeach
                @endforeach
            </div>
                    ...
                    <form action="{{ route('customer.addComment') }}" method="post">
                        ...
                        <ul class="list-inline" title="Average Rating">
                            @for($count = 1; $count<=5; $count++)
                                ...
                                @endif
                                <li title="đánh giá sao"
                                    id="{{ $product->sp_id }}-{{ $count }}"
                                    data-index="{{ $count }}"
                                    data-rating="{{ $rating }}"
                                    data-product="{{ $product->sp_id }}"
                                    data-customer="{{ Session::get('customer_id') }}"
                                    data-url="{{ route('customer.addComment') }}"
                                    class="rating"
